...
USE_GOOGLE_SHEETS = GSPREAD_AVAILABLE and "gcp_service_account" in st.secrets
...
if USE_GOOGLE_SHEETS:
    try:
        ...
        USE_GOOGLE_SHEETS = False
        st.warning(f"Không thể kết nối Google Sheets: {e}")
...
def save_data(df):
    ...
    if USE_GOOGLE_SHEETS:
        try:
            worksheet.clear()
            worksheet.append_row(list(DEFAULT_COLUMNS))
            for _, row in df.iterrows():
                worksheet.append_row(row.tolist())
            st.success("✅ Dữ liệu đã được lưu vào Google Sheets")
        except Exception as e:
        ...
        df.to_csv(csv_dir / "database.csv", index=False)
        st.success("✅ Dữ liệu đã được lưu vào CSV")
...
def save_product(name, code, category, cbm, packing, price, image_file):
    ...
    pd.concat([df, pd.DataFrame([new_row])], ignore_index=True).to_csv("temp.csv", index=False)
...
if not st.session_state["logged_in"]:
    ...
    if USE_GOOGLE_SHEETS:
        st.info("✅ Kết nối Google Sheets thành công! Dữ liệu sẽ được lưu trên cloud.")
    else:
        st.info("💾 Sử dụng chế độ CSV local. Để kích hoạt Google Sheets, xem GOOGLE_SHEETS_SETUP.md")
...
with header_col:
    ...
    if USE_GOOGLE_SHEETS:
        st.markdown("💾 *Dữ liệu: Google Sheets*")
    else:
        st.markdown("💾 *Dữ liệu: CSV Local*")
with logout_col:
